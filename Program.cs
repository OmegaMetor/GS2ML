...
if(File.Exists((workingDirectory + "\\ORIGINAL.exe")))
{
    Process.Start((workingDirectory + "\\ORIGINAL.exe"), "-game " + ".\\" + outputDataWin);
} else
{
    Console.WriteLine("\n\nERROR: \"ORIGINAL.exe\" FILE DOES NOT EXIST.\nPlease rename the ORIGINAL GameMaker exe file inside the path of the data.win to \"ORIGINAL.exe\" in order for this to work.");
}